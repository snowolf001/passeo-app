...
class PdfMaker: RCTEventEmitter {
  ...
  @objc func startJob(
    ...
    let margin = options["margin"] as? CGFloat ?? 20
    let maxPixel = options["maxPixel"] as? Int ?? 2200
    let jpegQuality = options["jpegQuality"] as? CGFloat ?? 0.82
  ...
  private func executeJob(
    ...
    for (index, imagePath) in imagePaths.enumerated() {
      autoreleasepool {
        ...
        guard let imageSource = loadImageSource(path: imagePath, maxPixel: maxPixel) else {
          sendError(jobId: jobId, message: "Failed to load image", index: index, path: imagePath)
          return
        }
        
        guard let image = UIImage(cgImage: imageSource) else {
          sendError(jobId: jobId, message: "Failed to create UIImage", index: index, path: imagePath)
          return
        }
  ...
  private func applyRotation(context: CGContext, rotation: Int, rect: CGRect) {
    ...
    // Apply rotation (CGContext uses radians)
