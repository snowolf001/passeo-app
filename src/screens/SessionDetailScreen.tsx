...
export default function SessionDetailScreen({route, navigation}: Props) {
  ...
  const {currentMembership, decrementCurrentMembershipCredits} = useApp();
  ...
  const isCheckedIn = currentMembership
    ? attendanceService.isCheckedIn(currentMembership.id, sessionId)
    : false;
  ...
  const handleSelfCheckIn = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Instant local update — no async work, no re-render cascade,
        // snackbar animation stays undisturbed
